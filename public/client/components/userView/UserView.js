  ...
  render() {
    ...
    if (this.state.userStars !== null) {
      ...
      for(var i = 0; i < userStars.length; i++) {
        if (!roundStars[userStars[i]._roundId]) {
          roundStars[userStars[i]._roundId] = [];
        }
        roundStars[userStars[i]._roundId].push(userStars[i]);
      }
    }
        ...
        <table className="container row">
          <tbody>
            {this.props.rounds.map( (round, i) => {
              return <Week key={i} round={round} stars={roundStars[round._id]} roundNum={i} />
            })}
          </tbody>
        </table>
      </div>
